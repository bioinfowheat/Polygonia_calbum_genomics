...
Ref_pal <- read_table("Cinxia_contigs.txt", col_names = F) %>%
   rename(Chr = X1) %>%
  filter(Chr != "Mitochondria") %>%
  mutate(color = rep(brewer.pal(n=11, "Spectral"), length.out = length(Chr)))

Qry_pal <-read_table2("Pcalbum_contigs.txt", col_names = F) %>%
  rename(Chr = X1) %>%
  mutate(color = "#FFFFFF")
...
Alignment <- read_csv("nucmer_aln_Mcin_v_Pca.qr1_filter.reformat.coords", col_names = nucmer_coord_colnames2 )
